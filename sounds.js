const availableSounds = {
    ...
    thunder: {
        title: "Imagine Dragons - Thunder  Felices sueños  🐸",
...
function createSoundSelector() {
    ...
    container.className = 'sound-selector';
    
    const title = document.createElement('h2');
    title.setAttribute('data-translate', 'selectSound');
    title.textContent = translations[currentLanguage].selectSound;
    ...
    Object.entries(availableSounds).forEach(([key, sound]) => {
        ...
        soundCard.innerHTML = `
            <img src="${sound.thumbnail}" alt="${sound.title}" onerror="this.src='placeholder.jpg'">
            <h3>${sound.title}</h3>
            <button class="btn" onclick="changeAlarmSound('${key}')">
...
// Cargar el sonido guardado al iniciar
